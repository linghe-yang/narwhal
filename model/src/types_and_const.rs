...
pub fn leader_round_to_epoch_index_tusk(round: Round, max_epoch: u64) -> (Epoch, usize) {
    assert_eq!(round % 2, 0, "round must be an even number");
    assert!(round > 0, "round must be positive");
    assert!(max_epoch > 0, "max_epoch must be positive");

    let rounds_per_epoch = 2 * max_epoch;
    let epoch = (round - 2) / rounds_per_epoch + 1;
    let index = ((round - 2) % rounds_per_epoch) / 2 + 1;

    (epoch, index as usize)
}

pub fn epoch_index_to_leader_round_tusk(epoch: Epoch, index: usize, max_epoch: u64) -> Round {
    assert!(epoch > 0, "epoch must be positive");
    assert!(index > 0 && index <= max_epoch as usize, "index must be between 1 and max_epoch");
    assert!(max_epoch > 0, "max_epoch must be positive");

    let rounds_per_epoch = 2 * max_epoch;
    let round = (epoch - 1) * rounds_per_epoch + 2 * index as u64;
